...
void mlfq() {
    ...
    for (int i = 0; i < total_time; ++i) {
        ...
        for (int j = 0; j < process_table_size; ++j) {
            if (process_table[j].arrival_time == i) {
                queue_enqueue(&(queues[0]), j);
                process_table[j].remain_time = tq0;
            }
        }

        if (!queue_is_empty(&(queues[0]))) {
            int index_of_process = queue_peek(&(queues[0]));
            gantt_chart_update(chart, &sz_chart, process_table[index_of_process].name, 1);
            if (--process_table[index_of_process].burst_time == 0) {
                queue_dequeue(&(queues[0]));
            }
            else if (--process_table[index_of_process].remain_time == 0) {
                queue_dequeue(&(queues[0]));
                queue_enqueue(&(queues[1]), index_of_process);
                process_table[index_of_process].remain_time = tq1;
            }

        } else if (!queue_is_empty(&(queues[1]))) {
            int index_of_process = queue_peek(&(queues[1]));
            gantt_chart_update(chart, &sz_chart, process_table[index_of_process].name, 1);
            if (--process_table[index_of_process].burst_time == 0) {
                queue_dequeue(&(queues[1]));
            }
            else if (--process_table[index_of_process].remain_time == 0) {
                queue_dequeue(&(queues[1]));
                queue_enqueue(&(queues[2]), index_of_process);
                process_table[index_of_process].remain_time = process_table[index_of_process].burst_time;
            }

        } else if (!queue_is_empty(&(queues[2]))) {
            int index_of_process = queue_peek(&(queues[2]));
            gantt_chart_update(chart, &sz_chart, process_table[index_of_process].name, 1);
            if (--process_table[index_of_process].burst_time == 0) {
                queue_dequeue(&(queues[2]));
            }
        }
