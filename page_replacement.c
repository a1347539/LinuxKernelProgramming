...
void algorithm_FIFO() {
    ...
    for (int i = 0; i < reference_string_length; ++i) {
        ...
        if (isInFrameTable) continue;

        frames[(total_page_fault++)%frames_available] = reference_string[i];
...
void algorithm_OPT() {
    ...
    int isInFrameTable;
    int distance[MAX_FRAMES_AVAILABLE];
    int numberOfTableRecord = 0;
    int total_page_fault = 0;
    
    for (int i = 0; i < reference_string_length; ++i) {
        clearArray(distance, MAX_FRAMES_AVAILABLE);
        isInFrameTable = 0;
        for (int j = 0; j < frames_available; ++j) {
            if (reference_string[i] == frames[j]) {
                // no page fault
        ...
        if (isInFrameTable) continue;
        ++total_page_fault;
        if (numberOfTableRecord < frames_available) {
            frames[numberOfTableRecord++] = reference_string[i];
        } else {
            for (int k = 0; k < frames_available; ++k) {
                for (int m = i; m <= reference_string_length; ++m) {
                    if (m == reference_string_length) {
                        distance[k] = MAX_REFERENCE_STRING;
                        break;
                    }
                    if (frames[k] == reference_string[m]) {
                        distance[k] = m - i;
                        break;
                    } 
                }
            ...
            for (int k = 1; k < frames_available; ++k) {
                if (distance[k] > distance[maxIndex]) { maxIndex = k; }
                else if (distance[k] == distance[maxIndex]) {
                    if (frames[maxIndex] > frames[k]) { maxIndex = k; }
                }
            }
            frames[maxIndex] = reference_string[i];
        }
